...
def _flatten_findings(results: Dict) -> List[Dict]:
    """Extract all offensive/finding entries from all known result buckets."""
    findings = []
    # All known buckets that may contain security findings
    all_buckets = [
        "offensive", "sqlmap", "xss", "ssrf", "idor", "ssti", "auth_matrix",
        "nosqli", "csrf", "lfi", "rce", "graphql", "host_header", "open_redirect",
        "cors", "path_traversal", "command_injection", "xxe", "prototype_pollution",
        "deserial", "request_smuggling", "race_condition", "mass_assignment",
        "crlf", "jwt", "ws_fuzz", "nuclei", "cve", "subdomain",
        "findings", "final", "scored_findings",
    ]
    seen = set()
    for bucket in all_buckets:
        raw = results.get(bucket, [])
        # scored_findings stores {"findings": [...], "total": N}
...
class PDFReporter:
    ...
    def _render_html(self, results: Dict, config: Dict) -> Optional[str]:
        ...
        _raw_confidence = waf.get("confidence", 0.0)
        try:
            waf_confidence_fmt = f"{float(_raw_confidence):.0%}"
        except (TypeError, ValueError):
            waf_confidence_fmt = str(_raw_confidence) if _raw_confidence else "0%"
        ...
        template_data = {
            ...
            "waf_confidence": waf_confidence_fmt,
